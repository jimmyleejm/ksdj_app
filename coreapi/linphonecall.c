...
static void linphone_call_set_terminated(LinphoneCall *call){
	...
	//jimmy this can be closed??
	//linphone_call_log_completed(call);
	//printf("linphone_call_log_completed\n");
...
//static void linphone_call_start_audio_stream(LinphoneCall *call, const char *cname, bool_t muted, bool_t send_ringbacktone, bool_t use_arc){
 void linphone_call_start_audio_stream(LinphoneCall *call, const char *cname, bool_t muted, bool_t send_ringbacktone, bool_t use_arc){
	LinphoneCore *lc=call->core;
	int used_pt=-1;
	...
	if (stream && stream->dir!=SalStreamInactive && stream->rtp_port!=0){
		...
		if (used_pt!=-1){
			...
			if (call->params.in_conference){
				//ransform the graph to connect it to the conference filter 
...
#endif
	if (call->audiostream!=NULL) {
			//jimmy ++ start audio test 
			//printf("call is %s\n cname is %s\n all_inputs_muted is %d \n send_ringbacktone is %d \n use_arc is %d \n",call->localip,cname,all_inputs_muted,send_ringbacktone,use_arc);
			#ifdef Doorphone_compile
			//if(call_glj_flag==0)
			//{

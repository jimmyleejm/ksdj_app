...
static void net_config_read (LinphoneCore *lc)
{
	...
	tmp=lp_config_get_int(lc->config,"net","mtu",1440); //1300
...
*/
...
#endif
	tmp=lp_config_get_int(lc->config,"sound","echocancellation",tmp);  //jimmy   echo cancellation
	...
	//linphone_core_set_playback_gain_db (lc,15.000000);
...
#ifdef VIDEO_ENABLED
//	linphone_core_assign_payload_type(lc,&payload_type_jpeg,26,NULL);	//Modify by EasyWave 20140214
	linphone_core_assign_payload_type(lc,&payload_type_h263,34,NULL);
//	linphone_core_assign_payload_type(lc,&payload_type_theora,97,NULL);	//Modify by EasyWave 20140214
	...
	linphone_core_assign_payload_type(lc,&payload_type_h264,102,"packetization-mode=1;profile-level-id=42001F");
//	linphone_core_assign_payload_type(lc,&payload_type_vp8,103,NULL);	//Modify by EasyWave 20140214
//	linphone_core_assign_payload_type(lc,&payload_type_x_snow,-1,NULL);	//Modify by EasyWave 20140214
...
**/
// jimmy ++ loop thread to deal with call station
...
LinphoneAddress * linphone_core_interpret_url(LinphoneCore *lc, const char *url){
	...
	//printf("linphone_address_new is \n");
	if (uri!=NULL){
		return uri;
	}
...
int linphone_core_accept_call_with_params(LinphoneCore *lc, LinphoneCall *call, const LinphoneCallParams *params)
{
	...
	if (!was_ringing && call->audiostream->ms.ticker==NULL){
		audio_stream_prepare_sound(call->audiostream,lc->sound_conf.play_sndcard,lc->sound_conf.capt_sndcard); //jimmy test
	}
	...
	linphone_core_update_streams(lc, call, new_md);  //jimmy
	if (new_md){
		linphone_call_set_state(call,LinphoneCallStreamsRunning,"Connected (streams running)");
...
/**
 * jimmy ++Set sound playback level in 0-100 scale
 *
 * @ingroup media_parameters
**/
...
void linphone_core_play_named_tone(LinphoneCore *lc, LinphoneToneID toneid){
	...
	if (linphone_core_tone_indications_enabled(lc)){
		...
		printf("linphone_core_play_named_tone!\n");
		if (f==NULL){
			//ms_error("No dtmf generator at this time !");
			printf("No dtmf generator at this time !\n");
			return;
		}

...
void linphone_core_update_streams(LinphoneCore *lc, LinphoneCall *call, SalMediaDescription *new_md){
	...
	if (new_md) {
		...
		linphone_call_start_media_streams(call,all_muted,send_ringbacktone); //jimmy
	}
...
static void call_accepted(SalOp *op){
	...
	if (md && !sal_media_description_empty(md) && !linphone_core_incompatible_security(lc,md)){
		...
		if (sal_media_description_has_dir(md,SalStreamSendOnly) ||
			...
			if (call->refer_pending)  //jimmy +++ refer the call
...
if (client_sockfd!=-1){
	printf("linphonec_out client_sockfd =%d\n",client_sockfd);
	 if (write(client_sockfd, "fresh window", strlen("fresh window")) == -1) {
			perror ("write");
		   // exit (EXIT_FAILURE);
		}
	}

...
static int lpc_cmd_redirect(LinphoneCore *, char *);
 int lpc_cmd_call_logs(LinphoneCore *, char *);
...
static LPC_COMMAND commands[] = {
	...
	{ "duration", lpc_cmd_duration, "Print duration in seconds of the last call.", NULL },//jimmy Print duration in seconds of the last call
	...
	{ "call-logs", lpc_cmd_call_logs, "Calls history", NULL },//jimmy +++    show the call  logs
...
#ifdef VIDEO_ENABLED //jimmy it can control the display ??
...
lpc_cmd_call(LinphoneCore *lc, char *args)
{
	...
	}
	{
		...
		opt1=strstr(args,"--audio-only");//audio-only
		opt2=strstr(args,"--early-media");//video-only
		if (opt1){
			opt1[0]='\0';
...
int rec_mag()
{
	...
	addlen=sizeof(struct sockaddr);
		...
		if((fp_w=fopen("/mnt/nand1-1/call_logs_data_rec","w"))==NULL)
	...
	while( strstr(recvline, "end") == NULL)
	{	
...
//	 printf("cr is %s\n",cr->peer);
	...
	if(cr==NULL)
		{
		printf("cr null 111111\n");

	     }
	printf("linphone_core_create_chat_room 111111\n");
...
//jimmy rec mag
	...
	if( (strstr(arg2, "call_logs_get")) != NULL)
		{
		rec_mag();
		linphonec_out("call_logs_get\n");
       }


	   if( (strstr(arg2, "call_ins_command")) != NULL)
			   {
			   printf("ins_call_flag set!\n");
			   sscanf(arg2,"call_ins_command,%d,%d,%d,%d",&foor_shi,&foor_ge,&hu_shi,&hu_ge);
			 
			   buff_rec[0]=foor_shi*16+foor_ge;
			   buff_rec[1]=hu_shi*16+hu_ge;
			   ins_call_flag=1;
...
lpc_cmd_transfer(LinphoneCore *lc, char *args)
{
	if (args){
		...
		if (n==1 || isalpha(*arg1)){
			...
			printf("linphone_core_transfer_call\n");
...
{
	printf("in the lpc_cmd_autoanswer\n");
	if ( ! args )
	{
	...
	if (strstr(args,"enable")){
		printf("linphonec_set_autoanswer\n");
...
lpc_cmd_reboot()
{
		 printf("lpc_cmd_reboot!!!!!!!\n");
		
		system("chmod 777 -R /mnt/nand ");
		
		system("cp /mnt/nand/videodoor.bin /mnt/nand1-1  ");
		// printf("main mtest2\n");
		system("cp /mnt/nand/mtest2 /mnt/nand1-1 ");
		//printf("main mtest2\n");
		system("cp /mnt/nand/go_videodoor.sh /mnt/nand1-1 ");
		system("cp /mnt/nand/route.sh /mnt/nand1-1 ");
		system("cp /mnt/nand/lib_romfs.bin /mnt/nand1-1 ");
		system("cp /mnt/nand/boot_videodoor /mnt/nand1-1 ");

		system("cp /mnt/nand/mycdev.ko /mnt/nand1-1 ");
...
int record_file(FILE*fp_w)
{
  ...
  int num_nums=0;
 /* 
	if((fp_w=fopen("/mnt/nand1-1/call_logs_data_backups","a+"))==NULL)
	//if((fp_w=fopen("call_logs_data_backups","a"))==NULL)
		{
		printf("can not open the file.\n");
		return -1;
		}
	...
	//if((fp_r=fopen("call_logs_data","r"))==NULL)
		...
		{		
		   memset(buf_call  , 0 , sizeof(buf_call) );
			 fgets( buf_call ,  sizeof(buf_call)  , fp_r );
	     if(num_line == num_mark*7+1)
	     	{
	     		num_mark++;
	     		//buf_call[0]=buf_call[0]+1;
	     		num_nums=atoi(buf_call);
	     		if(num_nums>0)
	     		{
	     		//printf("%d\n" , num_nums+1);
	     		fprintf(fp_w,"%d\n" , num_nums+1);
		...
		fclose(fp_w); 
		fclose(fp_r); 
		system("cp /mnt/nand1-1/call_logs_data_backups /mnt/nand1-1/call_logs_data");
		
		return 0;
}

//char frist_call_log[400];
...
lpc_cmd_call_logs(LinphoneCore *lc, char *args)
{
	...
	FILE*fp_w;
    int num=1;
	if((fp_w=fopen("/mnt/nand1-1/call_logs_data_backups","w"))==NULL)
		{
		printf("can not open the file.\n");
		return -1;
		}

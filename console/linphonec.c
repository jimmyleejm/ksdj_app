			...
			if(call_occupy_flag==0)
				{
					...
					call_state=3;
					 ...
					 if(call_ins_flag!=1)
					 	{
					 	// time_keeping_fun(60); //jimmy two times open clock?
...
struct sockaddr_in from_pc;
#define Devicelist 1
#define Callrecord 2
#define Opendoor 3
#define Dooralarm 4
#define Malfunction 5
...
if(vice_used==0)
{
        if(sa_flag)
        	...
        	{
        		if(Equipment_type==2 ||Equipment_type==3)
        			{
	        		vice_used=1;
					pthread_mutex_lock(&Device_mutex);
					lcd_clean();
        	...
        	{
        		if(Equipment_type==1 ||Equipment_type==3)
        			{
	        		vice_used=1;
					pthread_mutex_lock(&Device_mutex);
					lcd_clean();
        	...
        	if(sa_flag >=5)
        		{
                  ...
                  fputc(buff_4[0],output);
        				...
        				if(Equipment_type==1)
							...
							else if(Equipment_type==2 ||Equipment_type==3)
							{
								vice_used=1;
								pthread_mutex_lock(&Device_mutex);
								lcd_clean();
								disp_hanzi(0x80,"通话占用");
								fist_line=1;	
								pthread_mutex_unlock(&Device_mutex);	
								time_keeping_uart(60);
 ...
 while(1)
 	{
		...
		if(times_set<=0)
			...
			call_state=0;
			//ins_to_flag=0;
			if(memkou_call_sn==1 || Equipment_type!=1)
				{
				call_occupy_flag=0;
				}
			memkou_call_sn=0;
	...
	for(i=0;i<2;)
		{
		   ...
		   {
		   		FD_ZERO(&rset);
				FD_SET(sock_chk_fd, &rset);
				tm.tv_sec = 0;
				tm.tv_usec = 500000;

				if ( select(sock_chk_fd + 1, NULL, &rset, NULL, &tm) <= 0)
				{
					 printf("connect error err is %d\n",err);
					i++;
					if(i>=2)
						{
						FD_CLR(sock_chk_fd, &rset);
						close(sock_chk_fd);
						return -1;
						}
				}

				else if (FD_ISSET(sock_chk_fd, &rset))
				{
				   FD_CLR(sock_chk_fd, &rset);
				   break;
				}
...
int send_command_to_pc(int command,char * ip_src,char * ip_des,int tpye_src,int tpye_des)
{
		int sock_command;
		int err;
		struct sockaddr_in addr_command;
		char smsg_command[300];
		...
		bzero(&addr_command,sizeof(addr_command));
		addr_command.sin_family=AF_INET;
		addr_command.sin_addr.s_addr=inet_addr((char *)inet_ntoa(from_pc.sin_addr));
		addr_command.sin_port=htons(PORT);
		
		
			
		err=connect(sock_command,(struct sockaddr *)&addr_command,sizeof(addr_command));
		if(err==-1)
		{
		 ...
		 memset (&smsg_command, 0, sizeof (smsg_command));
		switch(command)
			{
			case Devicelist:
				printf("Devicelist\n");
				sscanf(smsg_command,"<Devicelist><IP=%s,Type=%d>\n",ip_src,tpye_src);
				
	        case Callrecord:
				printf("Callrecord\n");
				sscanf(smsg_command,"<Callrecord><IP=%s,Type=%d,IP=%s,Type=%d>\n",ip_src,tpye_src,ip_des,tpye_des);
				
	        case Opendoor:
				printf("Opendoor\n");
				sscanf(smsg_command,"<Opendoor><IP=%s,Type=%d,IP=%s,Type=%d>\n",ip_src,tpye_src,ip_des,tpye_des);
				
	        case Dooralarm:
				printf("Dooralarm\n");
				sscanf(smsg_command,"<Dooralarm><IP=%s,Type=%d>\n",ip_src,tpye_src);
				
	        case Malfunction:
				printf("Malfunction\n");
				sscanf(smsg_command,"<Malfunction><IP=%s,Type=%d>\n",ip_src,tpye_src);
				
	        default:printf("error\n");

			}
		//printf("connect with server...\n");
		send(sock_command,smsg_command,strlen(smsg_command),0);
...
//	fflush(stdout); 
  ...
  char * pHeadPtr;
   ...
   char ip[30];
	...
	bzero(&from_pc, sizeof(struct sockaddr_in));
	from_pc.sin_family = AF_INET;
	from_pc.sin_addr.s_addr = htonl(INADDR_ANY);
	from_pc.sin_port = htons(6001);
	...
	while(1)
	{
		//从广播地址接受消息
		memset(smsg,0,sizeof(smsg));
		int ret=recvfrom(sock, smsg, sizeof(smsg), 0, (struct sockaddr*)&from_pc,(socklen_t*)&len);
    ...
    printf("IP:%s\n", (char *)inet_ntoa(from_pc.sin_addr));
    printf("Port:%d\n", ntohs(from_pc.sin_port));   
...
//while(1)
{
 sockfd=socket(AF_INET,SOCK_STREAM,0);
	...
	addr_ser.sin_addr.s_addr=inet_addr((char *)inet_ntoa(from_pc.sin_addr));
			...
			sleep(2);
															  ...
															  pthread_mutex_unlock(&Device_mutex);
																...
																*/
																light_run(1);
																memset(tmp_call,0,sizeof(tmp_call));	
																sprintf(tmp_call,"call sip:192.168.7.20%d",1);
																printf("tmp_call is %s\n",tmp_call);
																swich_gpio_set(1,1,0,1);
																MIC_run(1);
																ortp_mutex_lock(&prompt_mutex);
...
int equipment_init()
{
	...
	if((fpr_eq=fopen("/mnt/nand1-1/config/equipment_num","r"))==NULL)
						{
						printf("can not open equipment_num file.\n");
						return -1;
						}
								
		fscanf(fpr_eq,"%d",&Equipment_type);
		fclose(fpr_eq);
		printf("Equipment_type is %d\n",Equipment_type);


			if(Equipment_type==1)//zhu
		{
		buff_2[0]=0xAB;
		}
	else if(Equipment_type==2)//fu1
		{
		buff_2[0]=0xAC;

		}
	else if(Equipment_type==3)//fu2
		{
		buff_2[0]=0xAD;

		}
	else if(Equipment_type==4)//wq
		{
...
int kpd_check()
{
	struct input_event data;
	  ...
	  int ret;
	  int max_num=0;
	  FILE*fp_cm,*fp_do,*fp_ip,*fp_num,*fp_mac,*fp_lock,*fp_door_num,*eq_fd;
						...
						else if(kpd_state == 1)
							{
								if(num_ch == 1)
									...
									if(Equipment_type!=4)
										...
										{
										fscanf(fp_door_num,"%d",&door_num_read[0]);
										...
										if(Equipment_type==1)
											...
											{
											sprintf(tmp_show,"192.168.8.%d%d%d",door_num_read[0],door_num_read[1],door_num_read[2]);

											}
										else if(Equipment_type==3)
											{
											sprintf(tmp_show,"192.168.9.%d%d%d",door_num_read[0],door_num_read[1],door_num_read[2]);

											}
											else if(Equipment_type==4)
											{
											sprintf(tmp_show,"192.168.7.21%d",door_num_read[0]);
											}
				...
				if(num_ch == 10 )
					...
					if(kpd_state==3)
						...
						NEXT:
							...
							{
							printf("set equipment num\n");
											
											
						
								if(data_num[0]==3 &&data_num[1]==1 &&data_num[2]==4 &&data_num[3]==5 &&
									data_num[4]==9 &&data_num[5]==6 )
									{
									printf("set equipment num in\n");
									kpd_state=4;
									//refresh_flag=1;
									lcd_clean() ;
									disp_hanzi(0x80,"设置设备类型");
									fist_line=1;
									set_num = 10;
									//put_time=0;
									max_num=1;
					...
					else if(kpd_state==4)
						{
								...
								if(data_pint==1 &&data_num[0]<5 &&data_num[0]>0)
								{
							

								Equipment_type=data_num[0];
								if((eq_fd=fopen("/mnt/nand1-1/config/equipment_num","w"))==NULL)
								{
								printf("can not open Equipment_type the file.\n");
								return -1;
								}

								fprintf(eq_fd,"%d",data_num[0]);
					 		    fclose(eq_fd);
							...
							if(Equipment_type!=4)
								...
								if(Equipment_type==1)
									...
									ret=SetIfAddr("eth0",tmp_ip,"255.255.255.0","192.168.7.0");	
											...
											{
											sprintf(tmp_ip,"192.168.8.%d%d%d",data_num[0],
									  data_num[1],data_num[2]);
											  ret=SetIfAddr("eth0",tmp_ip,"255.255.255.0","192.168.8.0");	

											}
										else if(Equipment_type==3)
											{
											sprintf(tmp_ip,"192.168.9.%d%d%d",data_num[0],
									  data_num[1],data_num[2]);
											ret=SetIfAddr("eth0",tmp_ip,"255.255.255.0","192.168.9.0");   
								...
								data_num[1],data_num[2]);
											...
											{
										fprintf(fp_ip,"/sbin/ifconfig eth0 192.168.8.%d%d%d up\n",data_num[0],
								data_num[1],data_num[2]);
								//fprintf(fp_ip,"/sbin/route add -net 224.0.0.0 netmask 240.0.0.0 dev eth0\n");
								fprintf(fp_ip,"/sbin/route add -net 192.168.0.0 netmask 255.255.0.0 dev eth0\n");
								fprintf(fp_ip,"/sbin/route add default gw \"192.168.8.1\" dev eth0\n");
								fprintf(fp_ip,"/sbin/route\n");
								fprintf(fp_ip,"/sbin/ifconfig eth0 192.168.8.%d%d%d up\n",data_num[0],
								data_num[1],data_num[2]);	
											...
											{
												 fprintf(fp_ip,"/sbin/ifconfig eth0 192.168.9.%d%d%d up\n",data_num[0],
								 data_num[1],data_num[2]);
								 //fprintf(fp_ip,"/sbin/route add -net 224.0.0.0 netmask 240.0.0.0 dev eth0\n");
								 fprintf(fp_ip,"/sbin/route add -net 192.168.0.0 netmask 255.255.0.0 dev eth0\n");
								 fprintf(fp_ip,"/sbin/route add default gw \"192.168.9.1\" dev eth0\n");
								 fprintf(fp_ip,"/sbin/route\n");
								 fprintf(fp_ip,"/sbin/ifconfig eth0 192.168.9.%d%d%d up\n",data_num[0],
								 data_num[1],data_num[2]); 
								...
								fprintf(fp_num,"%d",data_num[0]);
					 			fclose(fp_num);
								  
								  sprintf(tmp_ip,"192.168.7.21%d",data_num[0]);
								  printf("tmp_ip is  %s\n",tmp_ip);
								...
								fprintf(fp_ip,"/sbin/ifconfig eth0 192.168.7.21%d up\n",data_num[0]);
								//fprintf(fp_ip,"/sbin/route add -net 224.0.0.0 netmask 240.0.0.0 dev eth0\n");
								fprintf(fp_ip,"/sbin/route add -net 192.168.0.0 netmask 255.255.0.0 dev eth0\n");
								fprintf(fp_ip,"/sbin/route add default gw \"192.168.7.1\" dev eth0\n");
								fprintf(fp_ip,"/sbin/route\n");
							...
							if((call_ins_flag!=1) && (ins_to_flag!=1) )	
									{
										...
										if(Equipment_type==4)
											...
											if(data_pint==3 )
											{
														...
														memset(tmp_call,0,sizeof(tmp_call));
														sprintf(tmp_call,"call sip:192.168.7.%d%d%d",data_num[0],data_num[1],data_num[2]);
														//sprintf(tmp_call,"call sip:192.168.7.201");
														swich_gpio_set(1,1,0,1);
														MIC_run(1);
														ortp_mutex_lock(&prompt_mutex);
											...
											if(call_state==0)
												...
												if(call_occupy_flag==0)
													...
													ortp_mutex_lock(&prompt_mutex);
														if(data_num[0]==0)
															{
															sprintf(received_prompt,"chat sip:192.168.7.201 men_to_sn 0,%c,%c,%c",data_num[1]*16+data_num[2],data_num[3]*16+data_num[4],data_num[5]*16+data_num[6]);

															}
														else
															{
														sprintf(received_prompt,"chat sip:192.168.7.201 men_to_sn %c,%c,%c,%c",data_num[0],data_num[1]*16+data_num[2],data_num[3]*16+data_num[4],data_num[5]*16+data_num[6]);
															}
														//strcpy(received_prompt,"status register\0");
														//	 printf("Receiving command '%s'\n",received_prompt);kfflush(stdout);
														have_prompt=TRUE;
													...
													memset(tmp_call,0,sizeof(tmp_call));
													sprintf(tmp_call,"chat sip:192.168.7.%d%d%d call_ins_command,%d,%d,%d,%d",data_num[0],data_num[1],data_num[2]
													,data_num[3],data_num[4],data_num[5],data_num[6]);
													printf("call phone is %s\n",tmp_ip);
													ortp_mutex_lock(&prompt_mutex);
				...
				if(data.value ==0)
					{
						if(num_ch == 10 )
							...
							swich_gpio_set(0,0,1,1);
							 ...
							 {
							 		printf("call_snj_ringup is %d\n",call_snj_ringup);
								 if(call_snj_ringup==1)
									 {	
								 ortp_mutex_lock(&prompt_mutex);
...
//while(1);
	...
	int door_alarm_init()
			...
			FILE*fp_door_alarm ;
				...
				if((fp_door_alarm=fopen("/mnt/nand1-1/door_alarm_file","r"))==NULL)
				{
				printf("can not open door_alarm_init the file.\n");
				return -1;
				}
	
	
				fscanf(fp_door_alarm,"door_alarm_flag<%d>",&open_door_alarm_flag);
				printf("door_alarm_flag is %d\n",open_door_alarm_flag);
				fclose(fp_door_alarm);	 
...
static void *pipe_thread(void*p){
	...
	*/			  
				...
				if( (strstr(tmp, "answer_call")) != NULL)
					...
					else if( (strstr(tmp, "door_alarm_open")) != NULL)
					{
						//printf("door_alarm_open\n");
						
						open_door_alarm_flag=1;		
					}
					else if( (strstr(tmp, "door_alarm_close")) != NULL)
						{
							//printf("door_alarm_close\n");
						open_door_alarm_flag=0;	
		...
		const LinphoneAddress *from, const char *msg)
{
	...
	#ifdef Doorphone_compile
 if( (pHeadPtr=strstr(msg, "call_ins_command")) != NULL)
		{
		call_ins_flag=1;
		...
		if((call_ins_flag!=1) && (ins_to_flag!=1) )
		...
		else if( (pHeadPtr=strstr(msg, "glj_get_call")) != NULL)
			{
				...
				else if( (pHeadPtr=strstr(msg, "set_password")) != NULL)
		{
				if(Equipment_type==2 ||Equipment_type==3)
					{
						pHeadPtr += strlen("set_password ");
						for(i=0;i<8;i++)
						{
							get_passwd_num[i]=*pHeadPtr;
							pHeadPtr++;
						}
						memset(tmp_passwd,0,sizeof(tmp_passwd));
						sprintf(tmp_passwd,"/mnt/nand1-1/password/%c%c%c%c",get_passwd_num[0],get_passwd_num[1],get_passwd_num[2],get_passwd_num[3]);
						printf("tmp_passwd is %s\n",tmp_passwd);
						...
						fprintf(fp_pd,"%c,%c,%c,%c,",get_passwd_num[4],get_passwd_num[5],get_passwd_num[6],get_passwd_num[7]);
						fclose(fp_pd);
					}
		}
					else if( (pHeadPtr=strstr(msg, "dele_password")) != NULL)
		{
			if(Equipment_type==2 ||Equipment_type==3)
					{
					pHeadPtr += strlen("dele_password ");
					for(i=0;i<4;i++)
					{
						get_passwd_num[i]=*pHeadPtr;
						pHeadPtr++;
					...
					fclose(fp_pd);
							...
							else if( (pHeadPtr=strstr(msg, "wqj_open_door")) != NULL)
		{
						...
						if(mts_mkj_num==6)
							{
								...
								if(buff_men_call_sn[1]==0xa1 ||buff_men_call_sn[1]==0xa2)
									{
									buff_men_call_sn_men[0]=buff_men_call_sn_men[0]+0x2;
									printf("%c%c%c%c%c%c%c%c%c%c\n",0xAF,0xAF,0xAF,0xAF,buff_men_call_sn_men[0],buff_men_call_sn_men[1],0x00,buff_men_call_sn[0],0xAB,buff_men_call_sn[1]);
						
									}
							
						}
					else if(mts_mkj_num==7)
						{
						if(mts_wqj_flag>0)
						{
					   ...
					   printf("ip_point is %s",ip_point);
						...
						if(ip_point[1] == '>')
							...
							if(wgj_call_sn_falg==1 )
								...
								if(alarm_num==6)
							 	{
							 				if(buff_open_door[1]==0xa1 ||buff_open_door[1]==0xa2)
							 					{
							 					buff_open_door_men[0]=buff_open_door_men[0]+0x2;
												printf("%c%c%c%c%c%c%c%c%c%c\n",0xAF,0xAF,0xAF,0xAF,buff_open_door_men[0],buff_open_door_men[1],0x00,buff_open_door[0],0xDD,buff_open_door[1]);
												}

...
static void enc_preprocess(MSFilter *f){
	...
	params->qmax = 32;		// Default value = 51
	params->qmin = 12;		// Default value = 0
...
static void h264_nals_to_msgb( uint8_t *Buffer, uint32_t num_nals, MSQueue * nalus ) {
	...
	if(num_nals > s_u32IFrameBufSize){
		...
		s_pu8IFrameBuf = malloc(num_nals);
		if(s_pu8IFrameBuf){
...
static int enc_set_br(MSFilter *f, void *arg){
	...
	if (d->bitrate>=1024000){
		...
		d->vsize.width = MS_VIDEO_SIZE_CIF_W;	//S_VIDEO_SIZE_VGA_W;

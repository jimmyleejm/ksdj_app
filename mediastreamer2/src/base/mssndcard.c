...
MSSndCard * ms_snd_card_new_capture(MSSndCardDesc *desc){
	return ms_snd_card_new_with_name_capture(desc,NULL);
}

MSSndCard * ms_snd_card_new_playback(MSSndCardDesc *desc){
	return ms_snd_card_new_with_name_playback(desc,NULL);
}
...
MSSndCard * ms_snd_card_new_with_name_playback(MSSndCardDesc *desc, const char* name) {
...
MSSndCard * ms_snd_card_new_with_name_capture(MSSndCardDesc *desc, const char* name) {
	MSSndCard *obj= (MSSndCard *)ms_new(MSSndCard,1);
	obj->desc=desc;
	obj->name=name?ms_strdup(name):NULL;
	obj->data=NULL;

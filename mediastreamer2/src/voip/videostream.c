...
#include <ortp/zrtp.h>
/*
#ifdef __DISABLE_VIDEOOUT__
#undef __DISABLE_VIDEOOUT__
#endif
*/
//jimmy ++ free the output
...
static void configure_video_source(VideoStream *stream){
...
#if !defined(__DISABLE_VIDEOOUT__)	
	...
	printf("Setting sent vsize=%ix%i, fps=%f",vsize.width,vsize.height,fps);
...
int video_stream_start (VideoStream *stream, RtpProfile *profile, const char *rem_rtp_ip, int rem_rtp_port,
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamSendOnly){
		...
		printf("stream->source name is %s\n",stream->source->desc->name);
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamRecvOnly){
		...
		if (stream->tee2){
			...
			printf("stream->jpegwriter name is %s\n",stream->jpegwriter->desc->name);
		}
		if (stream->output!=NULL)
			{
			ms_connection_helper_link (&ch,stream->output,0,-1);
			printf("stream->output name is %s\n",stream->output->desc->name);
			}
...
void video_preview_start(VideoPreview *stream, MSWebCam *device){
	MSPixFmt format;
	float fps=(float)30;//(float)29.97;
	int mirroring=1;
	int corner=-1;
	...
	ms_filter_link(stream->source, 0, stream->output2, 0);
...
void video_preview_stop(VideoStream *stream){
	ms_ticker_detach(stream->ms.ticker, stream->source);
	ms_filter_unlink(stream->source,0,stream->output2,0);

	...
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
	...
	printf("stream->soundread name is %s\n",stream->soundread->desc->name);
	if (stream->volsend)
		{
		ms_connection_helper_link(&h,stream->volsend,0,0);
		printf("stream->volsend name is %s\n",stream->volsend->desc->name);
		}
	if (stream->dtmfgen_rtp)
		{
		ms_connection_helper_link(&h,stream->dtmfgen_rtp,0,0);
		printf("stream->dtmfgen_rtp name is %s\n",stream->dtmfgen_rtp->desc->name);
		}
	if (stream->send_tee)
		{
		ms_connection_helper_link(&h,stream->send_tee,0,0);
		printf("stream->send_tee name is %s\n",stream->send_tee->desc->name);
		}
	ms_connection_helper_link(&h,stream->ms.encoder,0,0);
	printf("stream->ms.encoder name is %s\n",stream->ms.encoder->desc->name);
	ms_connection_helper_link(&h,stream->ms.rtpsend,0,-1);
	printf("stream->ms.rtpsend name is %s\n",stream->ms.rtpsend->desc->name);
	...
	printf("stream->ms.rtprecv name is %s\n",stream->ms.rtprecv->desc->name);
	ms_connection_helper_link(&h,stream->ms.decoder,0,0);
	printf("stream->ms.decoder name is %s\n",stream->ms.decoder->desc->name);
	if (stream->plc)
		{
		ms_connection_helper_link(&h,stream->plc,0,0);
		printf("stream->plc name is %s\n",stream->plc->desc->name);
		}
	if (stream->dtmfgen)
		{
		printf("stream->dtmfgen name is %s\n",stream->dtmfgen->desc->name);
		}
	if (stream->volrecv)
		{
		ms_connection_helper_link(&h,stream->volrecv,0,0);
		printf("stream->volrecv name is %s\n",stream->volrecv->desc->name);
		}
	if (stream->recv_tee)
		{
		ms_connection_helper_link(&h,stream->recv_tee,0,0);
		printf("stream->recv_tee name is %s\n",stream->recv_tee->desc->name);
		}
	if (stream->equalizer)
		{
		ms_connection_helper_link(&h,stream->equalizer,0,0);
		printf("stream->equalizer name is %s\n",stream->equalizer->desc->name);
		}
	ms_connection_helper_link(&h,stream->soundwrite,0,-1);
	printf("stream->soundwrite name is %s\n",stream->soundwrite->desc->name);

	/*call recording part, attached to both outgoing and incoming graphs*/
	if (stream->recorder){
		//printf("stream->recorder\n");

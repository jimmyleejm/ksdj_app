...
#if defined(__VIDEODOOR_S__) || defined(__VIDEODOOR_DUALVIDEO__)
...
#if defined(__H264_HW_ENCODER__)

uint32_t u32BufPhyAddr = 0;
#define	VIDEO_PALETTE_YUV420P_MACRO	50
...
static void setv4l_start_capture(V4lState *s){

	unsigned int u32Capture = VIDEO_START;
	ms_message("User Start Capture");
	if (ioctl(s->fd, VIDIOCCAPTURE, &u32Capture) < 0) {
		ms_message("User Start Capture failed...");
		return;
	}
}
...
static void setv4l_stop_capture(V4lState *s){

	unsigned int u32Capture = VIDEO_STOP;
	ms_message("User Stop Capture");
	if (ioctl(s->fd, VIDIOCCAPTURE, &u32Capture) < 0) {
		ms_message("User stop Capture failed...");
		return ;
	}
}
...
static void setv4l_start_encoder(int frame, V4lState *s)
{
	...
	if (ioctl(s->fd, VIDIOCMCAPTURE, &vmap) < 0) {
		if (errno == EAGAIN)
			ms_error("setv4l_start_encoder: Cannot Sync");
		else
			ms_error("setv4l_start_encoder: VIDIOCMCAPTURE %s", strerror(errno));
		return;
	}
	return;
}
...
static int v4l_do_mmap(V4lState *s){
	...
	s->frame_ind=0;
	setv4l_start_encoder(s->frame_ind,s);
	setv4l_start_capture(s);
	return 0;
}
...
static int v4l_configure(V4lState *s)
{
   ...
   printf("v4l_configure!!!!!!!!!\n");
	...
	fps = s->fps;
	printf("v4l_configure: set SW fps to be : %d.\n", fps);
	return 0;
}	
...
int ms_to_v4l_pix_fmt(MSPixFmt p){
	...
	#if defined(__H264_HW_ENCODER__)
		case MS_YUV420P:
				return VIDEO_PALETTE_YUV420P_MACRO;		
	#else
		case MS_YUV420P:
...
static mblk_t * v4l_grab_image_mmap(V4lState *s){
...
#if defined(__H264_HW_ENCODER__)
	syncframe  = query_frame;
	while( (ioctl( s->fd, VIDIOCSYNC, &query_frame ) < 0) &&
	  (errno == EAGAIN || errno == EINTR) )
	{
		if(TryCnt >= 100) {
			ms_warning("v4l: fail sync frame: %d", query_frame );
			ms_warning("v4l: aborting on error in VIDIOCSYNC: %s", strerror( errno ) );
			return NULL;
		} else {
			usleep(2000);
		}
	...
	vbuf.index = syncframe;
	err=ioctl(s->fd, VIDIOCGCAPTIME, &vbuf);
	if ( err < 0 ) {
		ms_warning("v4l_grab_image_mmap: error in VIDIOCGCAPTIME: %s.",strerror(errno));
		usleep(1000);
		return NULL;
	}

	u32BufPhyAddr = vbuf.m.userptr;	/* encode physical address */
...
static void v4l_purge(V4lState *s){
	int i;
	int err;
	for (i=s->frame_ind;i<s->frame_ind;++i){
...
#endif	//defined(__VIDEODOOR_S__) || defined(__VIDEODOOR_DUALVIDEO__)

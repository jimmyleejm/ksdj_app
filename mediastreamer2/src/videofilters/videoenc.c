...
static void prepare(EncState *s){
	...
	{
		ms_message("Codec bitrate set to %i",c->bit_rate);//ms_message
		//c->bit_rate=(float)s->maxbr*0.6;
		printf("MJPEG Codec bitrate set to %i ...\n",c->bit_rate);
		c->width = s->vsize.width;  
		c->height = s->vsize.height;
		c->time_base.num = 1;
		c->time_base.den = (int)s->fps;
		c->gop_size=(int)s->fps*5; /*emit I frame every 5 seconds*/
		c->pix_fmt=PIX_FMT_YUV420P;//PIX_FMT_YUVJ420P
...
static uint16_t ms_ntohs(uint8_t **pbuf_ptr) {
	...
	*cqt=NULL;
	...
	while (err!=-1)
	{
		...
		if (err==0xda)
		{
			...
			uint16_t len = ms_ntohs(&bistream);
...
static void split_and_send(MSFilter *f, EncState *s, mblk_t *frame){
	...
	else if (s->codec==CODEC_ID_MJPEG)
	{
		...
		mjpeg_fragment_and_send(f,s,frame,timestamp,
								...
								lqt,
								cqt);
		if(lqt != NULL)    { freeb(lqt); }
		if(cqt != NULL)    { freeb(cqt); }
...
void __register_ffmpeg_encoders_if_possible(void){
	...
	{
		printf("register mjpeg encoder codec.\n");

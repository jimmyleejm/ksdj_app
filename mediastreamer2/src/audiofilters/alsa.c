...
static MSSndCard * alsa_card_new_playback(int id);
static MSSndCard * alsa_card_new_capture(int id);
...
static void alsa_card_detect(MSSndCardManager *m){
	...
	if (access(DSP_NAME,F_OK)==0){
		MSSndCard *card=alsa_card_new_playback(0);
		if (card!=NULL)
			ms_snd_card_manager_add_card(m,card);
	}
	for (i=1;i<10;i++){
		MSSndCard *card=alsa_card_new_capture(i);
		if (card!=NULL)
			ms_snd_card_manager_add_card(m,card);
	}
...
static unsigned int get_card_capabilities_playback(const char *devname){
	snd_pcm_t *pcm_handle;
	unsigned int ret=0;
...
static unsigned int get_card_capabilities_capture(const char *devname){
	snd_pcm_t *pcm_handle;
	unsigned int ret=0;
...
static MSSndCard * alsa_card_new_capture(int id)
{
	...
	obj=ms_snd_card_new_capture(&alsa_card_desc);
	ad=(AlsaData*)obj->data;
	...
	ad->mixdev=ms_strdup_printf("plughw:%i",id);
	{
		snd_mixer_t *mixer;
		mixer = alsa_mixer_open(ad->mixdev);
		if (mixer==NULL) {
	...
	obj->capabilities=get_card_capabilities_capture(ad->pcmdev);
...
static MSSndCard * alsa_card_new_playback(int id)
{
	...
	obj=ms_snd_card_new_playback(&alsa_card_desc);
	ad=(AlsaData*)obj->data;
	/* remove trailing spaces from card name */
	char *pos1, *pos2;
	pos1=ms_strdup(name);
	...
	/*check card capabilities: */
	obj->capabilities=get_card_capabilities_playback(ad->pcmdev);
	if (obj->capabilities==0){
		ms_warning("Strange, sound card %s does not seems to be capable of anything, retrying with plughw...",obj->name);
		/*retry with plughw: this workarounds an alsa bug*/
		ms_free(ad->pcmdev);
...
int alsa_write_set_nchannels(MSFilter *obj, void *data){
	int *n=(int*)data;
	//jimmy ++ channels 3  set Change_alsa_mode

	printf("set_nchannels is %d!!!!\n",*n);
	if(*n==3)
		{
		Change_alsa_mode=1;
...
void alsa_write_process(MSFilter *obj){
	...
	int samples;
	int err;
//	static int init_audio_flag=0;
	static int i=0;
	//printf("id is %d!!!!!!!!\n",obj->desc->id);
	//printf("name is %s!!!!!!!!\n",obj->desc->name);

	
	//printf("ad->nchannels is %d!!!!!!!!\n",ad->nchannels);
	//jimmy ++ channels 2 is ring   channels 1 is audio 
if(ad->nchannels==2 && Change_alsa_mode==0)
	{
	//printf("111111\n",ad->nchannels);
		if (ad->handle==NULL && ad->pcmdev!=NULL){
 ...
 else if(ad->nchannels==1 && Change_alsa_mode==1)
 	{
	//printf("222222\n",ad->nchannels);

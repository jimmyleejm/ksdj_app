...
static int configure_fd_capture(int fd, int bits,int stereo, int rate, int *minsz)
{
	int p=0,cond=0;
	int i=0;
	...
	p=AFMT_S16_LE;	//Modify by EasyWave 20130304
	...
	else //if(min_size<=blocksize)
	{
		int size_selector1=0;
		while ((blocksize >> size_selector1) != 1)size_selector1++;
		 /*compute selector blocksize = 1<< size_selector*/
		int frag1 = (2 << 16) | (size_selector1);
		if (ioctl(fd, SNDCTL_DSP_SETFRAGMENT, &frag1) == -1) {
			ioctl(fd, SNDCTL_DSP_GETBLKSIZE, &min_size);
		}
		else
		{
			/*it's working*/
			min_size=1 << (frag1&0x0FFFF);
			ms_warning("Max fragment=%x, size selector=%x block size=%i.",frag1>>16,frag1&0x0FFFF,min_size);
		}
	}
...
static int configure_fd_playback(int fd, int bits,int stereo, int rate, int *minsz)
{
	int p=0,cond=0;
	int i=0;
	int min_size=0,blocksize=2048;
	int err;
...
static void  oss_open_capture(OssData* d, int *minsz){
	int fd=open(d->pcmdev,O_RDWR|O_NONBLOCK);
	if (fd>0) {
		d->pcmfd_read=d->pcmfd_write=configure_fd_capture(fd, d->bits, d->stereo, d->rate, minsz);
		return ;
	}
	...
	if (d->pcmfd_read > 0) {
		d->pcmfd_read=configure_fd_capture(d->pcmfd_read, d->bits, d->stereo, d->rate, minsz);
	} else {
...
static void  oss_open_playback(OssData* d, int *minsz){
	int fd=open(d->pcmdev,O_RDWR|O_NONBLOCK);
	if (fd>0) {
		d->pcmfd_read=d->pcmfd_write=configure_fd_playback(fd, d->bits, d->stereo, d->rate, minsz);
		return ;
	}
	ms_warning ("Cannot open a single fd in rw mode for [%s] trying to open two",d->pcmdev);
	...
	if (d->pcmfd_write > 0) {
		d->pcmfd_write=configure_fd_playback(d->pcmfd_write, d->bits, d->stereo, d->rate, minsz);
	} else {
...
static MSSndCard *oss_card_new_capture(const char *pcmdev, const char *mixdev){
	MSSndCard *card=ms_snd_card_new_capture(&oss_card_desc);
	OssData *d=(OssData*)card->data;
...
static MSSndCard *oss_card_new_playback(const char *pcmdev, const char *mixdev){
	MSSndCard *card=ms_snd_card_new_playback(&oss_card_desc);
	OssData *d=(OssData*)card->data;
	d->pcmdev=ms_strdup(pcmdev);
	d->mixdev=ms_strdup(mixdev);
	card->name=ms_strdup(pcmdev);
...
static void oss_detect(MSSndCardManager *m){
	...
	if (access(DSP_NAME,F_OK)==0){
		MSSndCard *card=oss_card_new_playback(DSP_NAME,MIXER_NAME);
	...
	for(i=1;i<10;i++){
		...
		if (access(pcmdev,F_OK)==0){
			MSSndCard *card=oss_card_new_capture(pcmdev,mixdev);
			ms_snd_card_manager_add_card(m,card);
		}
	}
}

static void * oss_thread_capture(void *p){
	...
	if (d->pcmfd_read>=0) {
		ms_warning("Close read dsp thread...");
		close(d->pcmfd_read);
		d->pcmfd_read=-1;
	}
...
static void * oss_thread_playback(void *p){
	...
	while(d->write_started){
		if (d->pcmfd_write>=0){
			if (d->write_started) {
				...
				if (err==bsize){
					FD_ZERO( &writefds );
					FD_SET(d->pcmfd_write, &writefds );
					tv.tv_sec       = 0;
					tv.tv_usec      = 0;
					select(d->pcmfd_write + 1 , NULL , &writefds , NULL, &tv );
					if( FD_ISSET(d->pcmfd_write, &writefds )) {
					...
					do{			
						ioctl(d->pcmfd_write, SNDCTL_DSP_GETOSPACE , &info);			
						usleep(1);
					}while(info.bytes < bsize);
				}
				else
				{
					int sz;
				...
				if( FD_ISSET(d->pcmfd_write, &writefds ))
				{
					//printf("WRITE d->write_started ==FALSE .\n");
					memset(wtmpbuff,0,bsize);
					sz = write(d->pcmfd_write,wtmpbuff,bsize);
					if( sz!=bsize) ms_warning("sound device write returned %i !",sz);
				}	
	...
	if (d->pcmfd_write>=0) {
		ms_warning("Close write dsp thread...");
		close(d->pcmfd_write);
		d->pcmfd_write=-1;
	}
...
static void oss_start_r(MSSndCard *card){
	...
	if (d->read_started==FALSE){
		d->read_started=TRUE;
		ms_thread_create(&d->thread,NULL,oss_thread_capture,card);
...
static void oss_start_w(MSSndCard *card){
	...
	if (d->write_started==FALSE){
		...
		ms_thread_create(&d->thread_playback,NULL,oss_thread_playback,card);
	}else{
		d->write_started=TRUE;
	}
}
...
static void oss_stop_w(MSSndCard *card){
	...
	d->write_started=FALSE;
	if (d->write_started==FALSE){
		ms_thread_join(d->thread_playback,NULL);
	}
}
